...
const IBM_PAIN_API_URL = process.env.IBM_PAIN_API_URL || 'https://api.digitalhealth.ibm.com/pain-states/api/v1';
...
app.get('/api/pain-reports/:patientId', (req, res) => {
  ...
  try {
    ...
    fetchFromIBMPainStates(patientId, timeframe)
      ...
      .catch(err => {
        console.error('Error fetching from IBM API, falling back to local data', err);
  ...
  try {
    ...
    if (ibmResponse.success) {
      res.status(201).json({ 
        ...
        message: 'Pain report saved successfully to IBM Pain States API and locally'
      });
    } else {
      res.status(201).json({ 
        success: true, 
...
function generateLocalInsights(reports) {
  ...
  try {
    ...
    // Sort by timestamp
    painIntensities.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    
    // Calculate average pain
    const avgPain = painIntensities.reduce((sum, p) => sum + p.intensity, 0) / painIntensities.length;
    
    // Calculate trend
    const firstWeekIntensities = painIntensities.slice(0, Math.min(7, Math.ceil(painIntensities.length / 4)));
    const lastWeekIntensities = painIntensities.slice(-Math.min(7, Math.ceil(painIntensities.length / 4)));
    
    const firstWeekAvg = firstWeekIntensities.reduce((sum, p) => sum + p.intensity, 0) / firstWeekIntensities.length;
    const lastWeekAvg = lastWeekIntensities.reduce((sum, p) => sum + p.intensity, 0) / lastWeekIntensities.length;
    
    const trend = lastWeekAvg < firstWeekAvg ? 'improving' : 
                 lastWeekAvg > firstWeekAvg ? 'worsening' : 'stable';
    ...
    if (avgPain > 7) {
      recommendations.push('Your pain levels are high. Consider consulting with your healthcare provider soon.');
    }
    
    if (trend === 'worsening') {
      recommendations.push('Your pain appears to be increasing. Track any changes in activities or treatments.');
    } else if (trend === 'improving') {
      recommendations.push('Your pain appears to be improving. Continue with current management strategies.');
    }
    ...
    if (sleepImpact.length > 0) {
      const severeImpact = sleepImpact.filter(s => 
        s === 'severely affected' || s === 'cannot sleep'
      ).length;
      
      if (severeImpact / sleepImpact.length > 0.3) {
        recommendations.push('Your pain is significantly affecting your sleep. Consider discussing sleep management strategies with your healthcare provider.');